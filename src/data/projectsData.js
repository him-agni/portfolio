...
export const projectsData = [
  {
    ...
    demoTranscript: [
      `This demo introduces the SaaS Integration Hub: a centralized pipeline for payment, inventory, and delivery events that would otherwise be scattered across separate vendor dashboards.`,
      `The walkthrough shows the React dashboard and event simulator, then follows an inbound event through signature verification, normalization, and persistence in the MongoDB event store.`,
      `From the shared processor, the event fans out independently to Slack, email, and HubSpot. The dashboard records each action's outcome so a single provider failure does not hide what succeeded or block the remaining destinations.`,
      `The reliability controls highlighted in the demo are raw-body HMAC verification and idempotent processing. Duplicate Stripe deliveries are rejected by a unique database constraint before customer-facing actions can run twice.`,
      `The result is one observable event history that helps an operations team answer whether an order was charged, communicated, and routed without manually correlating multiple tools.`,
    ],
  ...
  },
  {
    ...
    demoTranscript: [
      `This demo presents the Release Intelligence Dashboard, which turns a completed GitHub Actions deployment into one explained release-health decision.`,
      `The workflow-run webhook is verified and normalized before four signal providers are queried in parallel: GitHub Actions, Sentry, PostHog, and GCP Logging. A failed provider is marked as degraded instead of preventing the evaluation.`,
      `The dashboard displays the deployment timeline, the individual signal results, and a deterministic health score. Fixed, inspectable penalties produce a green, amber, or red status and a recommendation such as ship, monitor, investigate, or consider rollback.`,
      `The walkthrough also demonstrates mock scenarios for healthy, watch, and incident releases, allowing the complete decision path to be evaluated without relying on live third-party credentials.`,
      `Each snapshot is persisted so the release owner has a durable explanation of what the system observed, why the release received its score, and which sources were unavailable at evaluation time.`,
    ],
